...
def extract_city(**kwargs):
    ...
    query = 'select c.city,c.state, c.country from "2024_mariano_gomez_schema".city c where c.state = \'Buenos Aires\''
    df = pd.read_sql(query, redshift_conn_string)
    
    path = os.path.join(output_parquet, 'extract_city2.parquet')
...
def transform_data(**kwargs):
    ...
    path = os.path.join(output_parquet, 'transform_data2.parquet')
